...
import 'dart:io';
import 'dart:math';
import 'package:bizmart/app/middlewares/auth_middleware.dart';
import 'package:bizmart/app/models/business_details_edit_api/address_edit_api.dart';
import 'package:bizmart/app/models/business_details_edit_api/businnes_edit_api.dart';
import 'package:bizmart/app/models/business_details_edit_api/gst_edit_api.dart';
import 'package:bizmart/app/models/business_details_edit_api/kyc_edit_api.dart';
import 'package:bizmart/app/models/business_details_edit_api/promot_edit_api.dart';
import 'package:bizmart/app/models/business_details_edit_api/service_edit_api.dart';
import 'package:bizmart/app/models/business_details_edit_api/sociallinks_edit_api.dart';
...
import 'package:bizmart/app/modules/book_e_service/widgets/payment_details_widget.dart';
import 'package:bizmart/app/modules/checkout/bindings/checkout_binding.dart';
import 'package:bizmart/app/modules/checkout/views/checkout_view.dart';
...
import 'package:bizmart/app/models/business_details_get_api/kyc_details_get.dart';
...
import 'package:bizmart/app/models/business_details_get_api/social_media_link_get.dart';
import 'package:bizmart/app/modules/global_widgets/circular_loading_widget.dart';
import 'package:bizmart/app/providers/laravel_provider.dart';
...
import 'checkout/controllers/razorpay_controller.dart';
import 'checkout/views/razorpay_view.dart';
...
class BusinessDetails extends StatefulWidget {
 ...
 final String sid;
  ...
  const BusinessDetails({Key key, this.cat_list, this.sid,}) : super(key: key);
...
class _BusinessDetailsState extends State<BusinessDetails> {
  ...
  void initState() {
    ...
    setState(() {
      PromoteAndBenefits().PromoteAndBenefitsData(widget.sid).then((val) {
        selt = val['data']['id'];
        nums = val['data']['id'];
        promote_data = val['data']['promote'].toString();
        benefits_data = val['data']['benefits'].toString();
        plan_status = val['plan'];
        if(plan_status !=null)
        {
          endDate = val['plan']['end_date'].toString();
        ...
        print(promote_data);
        print(benefits_data);
        if (promote_data == "Silver") {
          setState(() {
            selected = 0;
          });
        }
        if (promote_data == "Gold") {
          setState(() {
            selected = 1;
          });
        }
        if (promote_data == "Diamond") {
          setState(() {
            selected = 2;
          });
        }
        if (promote_data == null) {
          setState(() {
            selected;
          });
        }
        if (benefits_data == "Premium listing") {
          setState(() {
            num = 5;
          });
        }
        if (benefits_data == "Business leads") {
          setState(() {
            num = 6;
          });
        }
        if (benefits_data == "Verified seal(view)") {
          setState(() {
            num = 7;
          });
        }
        if (benefits_data == "Digital catelogue(view)") {
          setState(() {
            num = 8;
          });
        }
        if (benefits_data == "Search by name & category") {
          setState(() {
            num = 4;
          });
        }
        if (benefits_data == null) {
          setState(() {
            num;
          });
        }
    ...
    AllCategories().AllCategoriesData().then((val) {
      ...
      setState(() {
        ...
        //       {
        //         setState(() {
        //           String cat_name = categories[i]['name']['en'].toString();
        //           print("+++++++NAME CAT NAME");
        //           print(cat_name);
        //           print("END");
        //
    ...
    KycDetailsGet().KycDetailsGetData(widget.sid).then((val) {
      setState(() {
        pan_img = val['data']['pan_card_number'].toString();
        drive_img = val['data']['driving_licence_no'].toString();
        llp_img = val['data']['business_llp_number'].toString();
        fssai_img = val['data']['fssai_license_number'].toString();
        other_docu_img = val['data']['other_documents'].toString();
        trade_img = val['data']['trade_license_number'].toString();
        compantylic_img = val['data']['company_license_number'].toString();
        liq_img = val['data']['liquor_license_number'].toString();
        passport_img = val['data']['passport'].toString();
        print("GET API OF KYC");
        print(pan_img);
        print(drive_img);
        print(llp_img);
        print(fssai_img);
        print(other_docu_img);
        print(trade_img);
        print(compantylic_img);
        print(liq_img);
        print(passport_img);
        print("GET API OF KYC END");
      });
    });
    GstDetailsGet().GstDetailsGetData(widget.sid).then((val) {
      setState(() {
        register_gstn.text = val['data']['registered_with_gstn'].toString();
        register_under_gst.text =
            val['data']['registered_address_under_gst'].toString();
        register_pincode.text = val['data']['pincode'].toString();
        gstno.text = val['data']['gst_no_uin_of_registered'].toString();
        pan_umber.text = val['data']['pan_number'].toString();
      });
    });

    SocialMediaLinkGet().SocialMediaLinkGetData(widget.sid).then((val) {
      setState(() {
        facebook.text = val['data']['facebook_link'].toString();
        instagram.text = val['data']['instagram_link'].toString();
        twitter.text = val['data']['twitter_link'].toString();
        youtube.text = val['data']['youtube_link'].toString();
        linkedin.text = val['data']['linkedin_link'].toString();
        yahoo.text = val['data']['yahoo'].toString();
      });
    });
    BusinessDetailsGet().BusinessDetailsGetData(widget.sid).then((val) {
      setState(() {
        establish.text = val['data']['add_your_establishment'].toString();
        payment_method.text = val['data']['payment_method'].toString();
        website.text = val['data']['bussiness_website'].toString();
        add_awards.text = val['data']['add_awards'].toString();
        cert_file = val['data']['add_certificate'];
        img = val['data']['image'].toString();
        video = val['data']['video'].toString();
        PlayerController = VideoPlayerController.network(video);
    ...
    ServiceList().ServiceListData(widget.sid).then((val) {
      setState(() {


        print(val['data']);
        service_name.text = val['data']['name']['en'].toString();
        service_price.text = val['data']['price'].toString();
        unit_price = val['data']['price_unit'].toString();
        cat_id = val['data']['cat_id'];
        select_id = int.parse(cat_id);
        discount_service_price.text = val['data']['discount_price'].toString();
        service_duration.text = val['data']['duration'].toString();
        service_cat.text = val['data']['cat_id'].toString();
        service_desc.text = val['data']['description']['en'].toString();
        imagess = val['data']['image'].toString();
        print(
        ...
        print(
            "+++++++++++++++++++++++++++++++++++++++++++++++++++SERVICE DETAILS++++++++++++++++++++++++++++++++++++++++++++++++++++++++++");
        print(imageGile);
        print(service_unit.text);
        print(imagess);
        print(service_desc.text);
        print(dropdownValue);

      });
    });

    AddressData().AddressDataList(widget.sid).then((val) {
      setState(() {
        companyName = val['data']['company_name'].toString();
        addressDetails = val['data']['address'].toString();
        districtName = val['data']['district'].toString();
        cityName = val['data']['city'].toString();
        landmarkName = val['data']['landmark'].toString();
        mobileNumber = val['data']['mobile_number'].toString();
        pinCode = val['data']['pincode'].toString();
        company_name.text = companyName;
        addresss.text = addressDetails;
    ...
    // ServiceList().ServiceListData().then((val) {
    //   for (var i = 0; i < widget.cat_list.length; i++) {
    //     if (val['data']['cat_id'] == widget.cat_list[i]['id'].toString()) {
    //       print(i);
    //       print('textttttt');
    //       setState(() {
    //         cat_id_index = i;
    //         print(widget.cat_list[i]);
    //       });
    //     }
    //     items.add(
    //       DropdownMenuItem(
    //         value: widget.cat_list[i]['id'],
    //         child: Text(widget.cat_list[i]['name']['en'].toString()),
    //       ),
    //     );
    //   }
    // });
  ...
  List<DropdownMenuItem> buildDropdownTestItems(List data_lists) {
    // print("____________________Data__________________");
    // print(widget.cat_list);
    // print("____________________Data End__________________");
    // print(data_lists);
    // List<DropdownMenuItem> items = List();
    //   for (var i in data_lists) {
    //     if(cat_id == i['id'])
    //       {
    //         print("CAT INDEX");
    //         cat_id_index =1;
    ...
    ServiceList().ServiceListData(widget.sid).then((val) {
      for (var i = 0; i < data_lists.length; i++) {
        ...
        if(val['data'] != null) {
          if(val['data']['cat_id'] != null) {
            if (val['data']['cat_id'] == data_lists[i]['id'].toString()) {
              setState(() {
                print(data_lists[i]);
                _selectedTest = data_lists[i]['id'];
                cat_id = _selectedTest;
  ...
  Widget build(BuildContext context) {
    return DefaultTabController(
      ...
      child: Scaffold(
        ...
        Padding(
          ...
          child: InkWell(
            onTap: ()
            {
              ...
              print("+++++++++++++++++++++Submit service DETAILS+++++++++++++++++++++++++++");
              File _service_img = imageGile;
              String _service_name = service_name.text.toString();
             // String _service_cat = onChange == null ?select_id.toString(): _selectedTest['id'].toString();
              ...
              print("+++++++++++++++++++++IMAGE DETAILS+++++++++++++++++++++++++++");
              BusinessServiceEdit().BusinessServiceDataEdit(widget.sid,_service_discount,_service_desc,_service_name,_selectedTest == null?cat_id:_selectedTest,_service_price,_service_unit,_service_duration,_service_img).then((val)
              {
                setState(() {
                  isLoaders= false;
                });
             ...
             //  if( _service_cat.isNotEmpty&&_service_discount.isNotEmpty&&_service_desc.isNotEmpty&&_service_duration.isNotEmpty&&_service_name .isNotEmpty && _service_price.isNotEmpty&&_service_unit.isNotEmpty)
             //  {
             //    setState(() {
             //      isLoaders = true;
             //    });
             //    print("+++++++++++++++++++++IMAGE DETAILS+++++++++++++++++++++++++++");
             //    BusinessService().BusinessServiceData(_service_discount,_service_desc,_service_name,cat_id,_service_price,_service_unit,_service_duration,_service_img).then((val)
             //    {
             //      setState(() {
             //        isLoaders= false;
             //      });
             ...
             //  }
              // if(imageGile !=null)
              //   {
              //     if(_service_discount.isNotEmpty&&_service_desc.isNotEmpty&&_service_duration.isNotEmpty&&_service_name .isNotEmpty && _service_cat.isNotEmpty && _service_price.isNotEmpty&&_service_unit.isNotEmpty&& imagess !=null ?_service_img == null : _service_img !=null)
              //     {
              //       setState(() {
              //         isLoaders = true;
              //       });
              //       print("+++++++++++++++++++++IMAGE DETAILS+++++++++++++++++++++++++++");
              //       BusinessService().BusinessServiceData(_service_discount,_service_desc,_service_name,_service_cat,_service_price,_service_unit,_service_duration,_service_img).then((val)
              //       {
              //         setState(() {
              //           isLoaders= false;
              //         });
              ...
              //     }
              //   }
              // if(imagess.isNotEmpty &&imageGile ==null)
              //   {
              //     if(_service_discount.isNotEmpty&&_service_desc.isNotEmpty&&_service_duration.isNotEmpty&&_service_name .isNotEmpty && _service_cat.isNotEmpty && _service_price.isNotEmpty&&_service_unit.isNotEmpty&& imagess !=null ?_service_img == null : _service_img !=null)
              //     {
              //       setState(() {
        ...
        Padding(
          ...
          child: InkWell(
            onTap: ()
            {
              ...
              AddressSavedEdit().SaveAddressEdit(widget.sid,_compnay,_address,_district,_city,_pincode,_landmark,_mobile_number).then((val)
              {   setState(() {
                isLoaders =false;
              });
          ...
          child: InkWell(
            onTap: ()
            {
             ...
             // print(_video);
              ...
              BusinessDetaolsEdit().BusinessDetaolsDataEdit(widget.sid,_establishment,_payment,awards,_website,_image,_video,certificate).then((val)
              {
                setState(() {
                  isLoaderone =false;
                });
             ...
             // && img.isEmpty?_image!=null:_image==null &&video.isEmpty?_video!=null:_video==null&&cert_file.isEmpty?certificate!=null:certificate==null
             //  if(_establishment.isNotEmpty && _payment.isNotEmpty && awards.isNotEmpty&&_website.isNotEmpty)
             //  {
             //
             //
             //  }
        ...
        Padding(
          ...
          child: InkWell(
            onTap: ()
            {
              ...
              print(_linkedin);
              print(_yahoo);
              SocialMediaDataEdit().SocialMediaLinkEdit(widget.sid,_facebook,_instagram,_twitter,_youtube,_linkedin,_yahoo).then((val)
              {
                setState(() {
                  isLoadertwo =false;
                  selecttwo = true;
        ...
        Padding(
          ...
          child: InkWell(
            onTap: ()
            {
              ...
              GstDataEdit().GstDataFieldEdit(widget.sid,_register_gdtn, _register_address, _register_pincode, gst_no, pan).then((val)
              {
                setState(() {
                  isLoaderthree =false;
        ...
        Padding(
          ...
          child: InkWell(
            onTap: ()
            {
              ...
              KycDetailsEdit().KycDetailsDadtaEdit(widget.sid,Pan, driving, passport_file, liquir, trade, llp, company,documnet,Fssai).then((val)                  {
                setState(() {
                  isLoaderfour = false;
                  selectfour=true;
                  currentIndex =7;
                ...
                if(val ==200 || val ==201)
                  {
                    ...
                    Fluttertoast.showToast(
                        msg:"Kyc details saved sucessfully".toString(),
                        toastLength: Toast.LENGTH_SHORT,
                        gravity: ToastGravity.CENTER,
                        timeInSecForIosWeb: 1,
                        backgroundColor:Get.theme.colorScheme.secondary,
        ...
        Padding(
          ...
          child: InkWell(
            onTap: ()
            {
              ...
              String sid = widget.sid.toString();

              PromoteAndBenifitsEdit().PromoteAndBenifitsDataEdit(widget.sid,plan == null ?promote_data:plan ,benefits==null?benefits_data:benefits).then((val)
              {
                setState(() {
                  isLoadersix =false;
                  selectfive =true;
                ...
                if(plan == null?promote_data.isNotEmpty:plan.isNotEmpty && selected != null)
                {
                  ...
                  // Navigator.push(context, MaterialPageRoute(builder: (context)=>PaymentPage(sid:sid,selected:selected,mobileNumber:mobileNumber,)));
        ...
        //         children: [
        //         Text("Address",style: TextStyle(color:Get.theme.colorScheme.secondary,fontSize: 18 ),),
        //         TextFormField(
    ...
    return Container(
      child: Column(
       ...
       //_______________PAN CARD_____________________________
          ...
          Padding(
            padding:  EdgeInsets.only(left: 20.0,right: 20,top: 5, ),
            child: Text("Passort",style: TextStyle(color: Get.theme.colorScheme.secondary,fontSize: 12),),
          ),
          Padding(
            padding:  EdgeInsets.only(left: 20.0,right: 20,top: 10, bottom: 10),
            child: InkWell(
              ...
              child: Container(
               // height: 50,
                ...
                child:llp==null && llp_img == null ? Row(
                      children: [
                        Icon(Icons.download_rounded, color: Color(0xff8ba3bd),),
                        SizedBox(width: 10,),
