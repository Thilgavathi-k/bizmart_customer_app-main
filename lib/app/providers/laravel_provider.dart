...
class LaravelApiClient extends GetxService with ApiClient {
  ...
  Future<User> login(User user) async {
   final prefs = await _prefs;
    ...
    if (response.data['success'] == true) {
      ...
      // var userId = response.data['data']['id'].toString();
       await prefs.setString('userId', response.data['data']['id'].toString());
       var userId = await prefs.getString('userId');
      print("DATA USER DETAILS___________________________");
      print(userId);
      print(response.data);
  ...
  Future<int> forgetPassword(User user) async
  {
    final prefs = await _prefs;

    Uri _uri = getApiBaseUri("forget_password");
    Get.log(_uri.toString());
  ...
  Future<User> ptpRegister(User user) async {
    final prefs = await _prefs;

    Uri _uri = getApiBaseUri("otp_register");
    Get.log(_uri.toString());
    Get.log(user.toString());
    var response = await _httpClient.postUri(
      _uri,
  ...
  Future<User> otpCheckPhone(User user) async {
    final prefs = await _prefs;

    Uri _uri = getApiBaseUri("otp_check");
    Get.log(_uri.toString());
    Get.log(user.toString());
    var response = await _httpClient.postUri(
      _uri,
  ...
  Future<EService> getEService(String id) async {
   ...
   // Uri _uri = getApiBaseUri("e_services/$id");
    Get.log(_uri.toString());
    var response = await _httpClient.getUri(_uri, options: _optionsNetwork);
    print("++++++++++++eservice deatils++++++++++++++++++++++++");
    print(_uri);
    print(response);
    print("++++++++++++eservice deatils end++++++++++++++++++++++++");
